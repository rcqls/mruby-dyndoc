#require File.join(File.dirname(__FILE__),'strscan_dyndoc')
...
module Dyndoc
  class Scanner
    ...
    def token_stack
      ...
      once=true
#      p [:token_stack,@scan] 
      while once || mode>-2 #since begin end while does not work
        once = false
#        p [:mode,mode,@scan]
        if mode != -1
    ...
    def init_atom
      if @tag[:atom]
        # p ["init_atom @txt"]
        # p ["init_atom @txt",@txt]
        @txt=@txt.gsub(@tag[:atom][:match]) do |w| 
    ...
    def extract(res=nil,txt=nil,type=nil)
      if res
        ...
        res2=res.map{|r|
          ...
          start=r[:stop]+1
          if r[:inside].empty?
#            p [:inside_empty,txt[(r[:start])..(r[:stop])]]
 ...
 #           p [:inside,r[:inside]]
        ...
        #p [:txt,start,txt,txt.class]
        #p [:txt2,txt2,txt2.class]
        txt2 += txt[start..-1]
        {:txt=>txt2,:type=>type,:inside=> res2}
      else
        res=@token[:inside].dup
        #p [:extract,extract(res,@token[:txt])]
    ...
    @@dtag={
      :dtag => {
        ...
        :keyword=>{
      	...
      	:with_tagblck =>[:document,:if,:unless,:for,:loop,:case,:set,:def,:meth,:new,:super,:do,:out,:blck,:saved,:call,:style,:rverb,:rout,:tags,:keys,:part],
      	:named_tag=>{
      #=begin
      	  :> => {:tag=>'(_TAG_)(?:[^\]]+)?',:rest=>/^>([^\]]*)$/},
      	  :>> => {:tag=>'(_TAG_)(?:[^\]]+)?',:rest=>/^>>([^\]]*)$/},
...
#p ["key=2",key,@tag_selected]
        ...
        ##Dyndoc.warn "TOOOOOOOOOOO",
#        p [:pre_match,@scan,[@scan.pre_match,@scan.matched,@scan.post_match],from,to,@scan.matched,@block,@block[from...to]] if @scan.matched=="[#>]"
        res=@block[from...to]
      else
#p ["pre_match=",@scan.pre_match,"to=",@scan.pre_match.size]
        to=@scan.pre_match.length
...
#p ["to_tmp=",@scan.pre_match,to_tmp]
...
#p ["res=",from,to,res,@block[from...to]]
      end
    inside2=[]
    #p [:res,pre,@is_arg,{:txt=>res,:inside=>inside2}]
    ...
    def check_until_for_named_tags
      #if res
      #p [:check_until_for_named_tags,@scan.rest,@named_tags]
      @named_tags.each_index{|i|
        #p [:check_until_for_named_tags,@scan[3+i]]
    	if @scan[3+i] and !@scan[3+i].empty?
...
#p ["last",/(#{@tag[:block]}?)\s*(#{@tag[:stop]})/,@scan[2],@scan[0],@scan[1]]
#p [from,@scan.pos,@scan.string,@scan.rest,@scan.pre_match,@scan.matched,@scan.post_match]
#p [:args,from,@scan[2],inside]
          res << find_text(from,@scan[2],inside)
#p ["convert_block: res",res]
          from=false
        end
#puts "from2";p from
      end
...
#      p ["parse_block: blck",blck] 
      res=convert_block(blck)
#      p ["parse_block: res",res] 
...
#p  ["blck",blck]
      res=[]
#      p [:re_starnge2,@re_strange2,blck[:txt],blck[:txt].split(@re_strange2,-1)]
      blck[:txt].split(@re_strange2,-1).each{|e|
#        p ["parse_text: e",e,@strange]
        if e==@strange
          b=blck[:inside].shift
#          p ["parse_text: b",b]
          res_b=parse_block(b)
#          p ["parse_text: res_b",res_b]
...
#puts "inside";p res[:inside]
      res[:inside].map do |e|
        vars2= [e[:type][2..-1].strip,(e[:inside] ? make_vars(e) : e[:txt][(e[:type].length+1)...-1] )] #instead of length
        vars << vars2
      end
#puts "vars";p vars
      vars
